...
<script>
            ...
            if (startQuizBtn && confirmStartQuizBtn) {
                startQuizBtn.addEventListener('click', function() {
                    ...
                    if (maxAttempts > 0 && currentAttempts >= maxAttempts) {
                        queueNotification('Attempt Limit', 'You have reached the maximum number of attempts for this quiz.');
                        return;
                    }
                });
                        ...
                        .then(text => {
                            ...
                            try {
                                ...
                                if (data.success) {
                                    ...
                                    queueNotification('Error', `Failed to start quiz: ${data.error || 'Unknown error'}`);
                                }
                            } catch (e) {
                                console.error('Failed to parse JSON:', e, 'Raw text:', text);
                                queueNotification('Error', 'Invalid server response. Please try again.');
                            }
                        })
                        .catch(error => {
                            console.error('Error starting quiz:', error);
                            queueNotification('Error', 'An error occurred while starting the quiz.');
                        });
                });
            }
            // Resume Quiz
    ...
    function startCountdown() {
        ...
        console.log('Starting countdown:', {
            activeAttemptDiv: !!activeAttemptDiv,
            activeTimer: !!activeTimer,
            modalTimer: !!modalTimer,
            initialTimeRemaining: activeAttemptDiv ? activeAttemptDiv.getAttribute('data-time-remaining') : null
        });
        ...
        if (activeAttemptDiv && activeTimer) {
            ...
            const countdown = setInterval(() => {
                if (timeRemaining <= 0) {
                    ...
                    console.log('Countdown ended, auto-submitting if quiz is active');
            ...
            console.warn('Countdown not started: missing elements');
        }
    }
