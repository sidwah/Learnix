...
try {
    ...
    $query = "
        SELECT
            ...
            CASE 
                WHEN c.status = 'Draft' AND c.approval_status = 'Approved' THEN 1
                ELSE 0
            END AS is_suspended
        FROM
            courses c
        ...
        WHERE
            ...
            (c.status = 'Draft' AND c.approval_status = 'Approved') OR
            (c.approval_status = 'Rejected')
        ORDER BY
            CASE 
                WHEN c.approval_status = 'Pending' AND c.status = 'Published' THEN 1
                WHEN (c.status = 'Draft' AND c.approval_status = 'Approved') THEN 2
                WHEN c.approval_status = 'Rejected' THEN 3
                ELSE 4
            END,
            crr.created_at DESC, 
            c.created_at DESC
