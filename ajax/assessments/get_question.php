...
try {
    ...
    // Verify instructor's access to the quiz through course_instructors table
    $access_check_query = "
        SELECT 
            sq.quiz_id,
            cs.course_id
    ...
    if ($question_id) {
        ...
        $stmt->bind_param("ssissii", $question_text, $question_type_enum, $points, $explanation, $user_id, $question_id, $quiz_id);
        ...
        $insert_query = "INSERT INTO quiz_questions (
            ...
            (SELECT IFNULL(MAX(question_order), 0) + 1 FROM quiz_questions q2 WHERE q2.quiz_id = ?))";
            
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param("issisii", $quiz_id, $question_text, $question_type_enum, $points, $explanation, $user_id, $quiz_id);
