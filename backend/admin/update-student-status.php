...
try {
    ...
    $query = "SELECT u.user_id, u.email, u.status, u.first_name, u.last_name 
              FROM users u
    ...
    $activity_type = "student_status_changed";
    ...
    <body>
        <div class="email-container">
            ...
            <div class="email-body">
                ...
                <p>This is to inform you that your student account status has been updated.</p>
                
                <p>Your account is now: <span class="status-badge" style="background-color: ' . $badge_color . ';">' . ucfirst($new_status) . '</span></p>
                ';
                
    if ($new_status === 'active') {
        $message .= '<p>You now have full access to the Learnix platform and can log in normally.</p>';
    } else if ($new_status === 'banned') {
    ...
    $text_message = "Hello " . $student['first_name'] . ",\n\n" .
                    "This is to inform you that your student account status has been updated.\n\n" .
                    "Your account is now: " . ucfirst($new_status) . "\n\n";
                    
    if ($new_status === 'active') {
        $text_message .= "You now have full access to the Learnix platform and can log in normally.\n\n";
    } else if ($new_status === 'banned') {
    ...
    $response = [
        'status' => 'success',
        'message' => 'Student status updated to ' . ucfirst($new_status) . ' successfully'
    ];
