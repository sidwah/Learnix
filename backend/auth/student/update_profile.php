...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        // Insert notification after successful profile update
        $notificationSql = "
...
";
        ...
        $title = 'Profile Update'; // ✅ New line
        $type = 'Profile Update';
        $message = 'Your profile information was updated successfully.';

        $notificationStmt->bind_param("isss", $user_id, $title, $type, $message);

        if (!$notificationStmt->execute()) {
            error_log("Failed to insert profile update notification: " . $conn->error);
        }
