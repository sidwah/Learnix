...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($email) && !empty($code)) {
        ...
        $sql = "SELECT * FROM student_reset_codes WHERE email = ? AND code = ? AND expiry_time > NOW()";
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            $userSql = "SELECT user_id FROM users WHERE email = ? AND role = 'student'";
            ...
            if ($userRow = mysqli_fetch_assoc($userResult)) {
                ...
                $resetAttemptsSql = "DELETE FROM student_reset_attempts WHERE email = ?";
                ...
                // Clear any lockouts
                $clearLockoutSql = "DELETE FROM student_reset_lockouts WHERE email = ?";
            ...
            $checkAttemptsSql = "SELECT * FROM student_reset_attempts WHERE email = ?";
            ...
            if ($attemptsRow = mysqli_fetch_assoc($checkAttemptsResult)) {
                ...
                $updateAttemptsSql = "UPDATE student_reset_attempts SET attempts = ?, last_attempt = NOW() WHERE email = ?";
                ...
                if ($newAttempts >= MAX_RESET_ATTEMPTS) {
                    ...
                    $checkLockoutSql = "SELECT * FROM student_reset_lockouts WHERE email = ?";
                    ...
                    if (mysqli_fetch_assoc($checkLockoutResult)) {
                        ...
                        $updateLockoutSql = "UPDATE student_reset_lockouts SET lockout_until = ?, attempts = attempts + 1 WHERE email = ?";
                        ...
                        // Create new lockout
                        $createLockoutSql = "INSERT INTO student_reset_lockouts (email, lockout_until, attempts) VALUES (?, ?, 1)";
                ...
                $createAttemptSql = "INSERT INTO student_reset_attempts (email, attempts, last_attempt) VALUES (?, 1, NOW())";
