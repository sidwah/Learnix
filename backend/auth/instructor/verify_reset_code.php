...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($email) && !empty($code)) {
        ...
        $sql = "SELECT * FROM instructor_reset_codes WHERE email = ? AND code = ? AND expiry_time > NOW()";
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            $userSql = "SELECT user_id FROM users WHERE email = ? AND role = 'instructor'";
            ...
            if ($userRow = mysqli_fetch_assoc($userResult)) {
                // Store the user_id in session temporarily
                ...
                $resetAttemptsSql = "DELETE FROM instructor_reset_attempts WHERE email = ?";
                ...
                // Clear any lockouts
                $clearLockoutSql = "DELETE FROM instructor_reset_lockouts WHERE email = ?";
            ...
            $checkAttemptsSql = "SELECT * FROM instructor_reset_attempts WHERE email = ?";
            ...
            if ($attemptsRow = mysqli_fetch_assoc($checkAttemptsResult)) {
                ...
                $updateAttemptsSql = "UPDATE instructor_reset_attempts SET attempts = ?, last_attempt = NOW() WHERE email = ?";
                ...
                if ($newAttempts >= MAX_RESET_ATTEMPTS) {
                    ...
                    $checkLockoutSql = "SELECT * FROM instructor_reset_lockouts WHERE email = ?";
                    ...
                    if (mysqli_fetch_assoc($checkLockoutResult)) {
                        ...
                        $updateLockoutSql = "UPDATE instructor_reset_lockouts SET lockout_until = ?, attempts = attempts + 1 WHERE email = ?";
                        ...
                        // Create new lockout
                        $createLockoutSql = "INSERT INTO instructor_reset_lockouts (email, lockout_until, attempts) VALUES (?, ?, 1)";
                ...
                $createAttemptSql = "INSERT INTO instructor_reset_attempts (email, attempts, last_attempt) VALUES (?, 1, NOW())";
