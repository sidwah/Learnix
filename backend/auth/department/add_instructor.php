...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Debug: Check if name fields are null or empty
    error_log("Debug - firstName: " . ($firstName === '' ? 'EMPTY' : $firstName));
    error_log("Debug - lastName: " . ($lastName === '' ? 'EMPTY' : $lastName));
    error_log("Debug - firstName is null: " . (is_null($_POST['firstName'] ?? null) ? 'YES' : 'NO'));
    error_log("Debug - lastName is null: " . (is_null($_POST['lastName'] ?? null) ? 'YES' : 'NO'));
    ...
    if (empty($firstName)) {
        ...
        exit(json_encode([
            ...
            'debug' => [
                'firstName_empty' => true,
                'firstName_value' => $firstName,
                'firstName_null' => is_null($_POST['firstName'] ?? null)
            ]
        ]));
    }
    ...
    if (empty($lastName)) {
        ...
        exit(json_encode([
            ...
            'debug' => [
                ...
                'lastName_null' => is_null($_POST['lastName'] ?? null)
            ]
        ]));
    }
    ...
    if (empty($email)) {
        ...
        exit(json_encode([
            ...
            'debug' => [
                'email_empty' => true,
                'email_value' => $email,
                'email_null' => is_null($_POST['email'] ?? null)
            ]
        ]));
    }
    ...
    try {
        ...
        // Get department-specific invitation expiry hours, fallback to 48 if not set
        ...
        // $expiryTime = date('Y-m-d H:i:s', strtotime('+' . $invitationExpiryHours . ' hours'));
                ...
                if ($result->num_rows > 0) {
                    ...
                    if (strtotime($existingInvitation['expiry_time']) > time()) {
                        ...
                        $stmt = $conn->prepare("UPDATE instructor_invitations 
                                       SET first_name = ?, last_name = ?, temp_password_hash = ?, invited_by = ?, expiry_time = ?, notes = ? 
                                       WHERE id = ?");
                        $stmt->bind_param("ssisssi", $firstName, $lastName, $passwordHash, $departmentHeadId, $expiryTime, $notes, $existingInvitation['id']);
                        $stmt->execute();
        ...
        <div class="email-container">
            ...
            <div class="email-body">
                ...
                <div class="expiry-alert">
                    <strong>⏱️ Time Sensitive:</strong> This invitation will expire in ' . $invitationExpiryHours . ' hours. Please log in and set up your account before the invitation expires.
                </div>
...
" : "") . "This invitation will expire in {$invitationExpiryHours} hours. Please log in and set up your account before the invitation expires.
